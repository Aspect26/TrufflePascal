...
public final class ReadVariableNodeFactory implements NodeFactory<ReadVariableNode> {

    private static ReadVariableNodeFactory instance;

    private ReadVariableNodeFactory() {
    }

    @Override
    public Class<ReadVariableNode> getNodeClass() {
        return ReadVariableNode.class;
    }
    ...
    public List getNodeSignatures() {
        return Arrays.asList(Arrays.asList(FrameSlot.class, TypeDescriptor.class));
    }

    @Override
    public ReadVariableNode createNode(Object... arguments) {
        if (arguments.length == 2 && (arguments[0] == null || arguments[0] instanceof FrameSlot) && (arguments[1] == null || arguments[1] instanceof TypeDescriptor)) {
            return create((FrameSlot) arguments[0], (TypeDescriptor) arguments[1]);
        } else {
            throw new IllegalArgumentException("Invalid create signature.");
        }
    }

    public static NodeFactory<ReadVariableNode> getInstance() {
        if (instance == null) {
            instance = new ReadVariableNodeFactory();
        }
        return instance;
    }
    ...
    @GeneratedBy(ReadVariableNode.class)
    public static final class ReadVariableNodeGen extends ReadVariableNode implements SpecializedNode {
